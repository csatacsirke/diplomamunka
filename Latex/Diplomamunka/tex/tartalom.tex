...
Egy ilyen megoldást készített a Jura Trade kft.\footnote{\url{http://www.juratrade.hu/}} is. Ezen dolgozat célja ennek az alkalmazásnak
...
A kanonikus gépi tanulási feladat a következőképpen írható fel:
\[ 
%\min\limits_{\theta} L(\theta, x_i, y_i) = \frac{1}{n}  \sumn{i} l(f(\theta, x_i), y_i) + R(\theta)
\min\limits_{\theta} L(\theta, \underline{x}, \underline{y}) = \frac{1}{n}  \sumn i l(f(\theta, x_i), y_i) + R(\theta)
 \]
, ahol $ \underline{x} $ a bemenet, $ \underline{y} $ a hozzá tartozó kimenet, $ f $ a modell, $ l $ a költségfüggvény, $ R $ a regularizáció, $ \theta $ a tanulandó paraméterek.



Manapság egyre több területen használják automatizálásra, például kép-, hang- és nyelvfeldolgozásban, gráf adatokon, Megerősítéses Tanulásban.% és egyéb automatizálásban mintafelismerésre: gyógyászat, önvezető autók, nyomtatott szöveg felismerése; klaszterezésre: adatbányászat, szociális hálók; vagy 
...
A Támasztóvektor Gép (Support Vector Machine, SVM) egy bináris osztályozási feladat. 
...
%TODO itt mutassunk bázisfüggvényeket
%ez a megvalósítás részbe került
% de az rbf ez lehet hogy ide lehetn rakni?...


\subsubsection{Mesterséges mély neurális hálózatok}
% TODO többszörösen összetett fv alakú alapú modell, amiben lévő függvények a rétegek. Ezek paraméteres leképezések, aminek paramétereit gradiens módszerrel számoljuk.
...
% TODO dense+konvolucios rétegről ábra
...
% TODO konv azért jü, mert csökkenti a paraméterek számát, ezért kevésbé tanul túl, a pixelek szomszédságát kihasználja, a dense nem
...
Egy mély háló modellje egy többszörösen összetett függvény, melynek függvényei a rétegek. Ezek paraméteres leképzések, melyek paramétereit gradiens módszerrel számoljuk. Általánosan, egy $ K $ rétegű modell a következőképpen írható fel:
%Egy mély háló modellje a következő alakban írható fel:

\[ 
f(\theta,\underline{x})=f_K(\theta_K, \dots f_2(\theta_2, f_1(\theta_1, \underline{x}) ) \dots )
 \]
...
Ennek megoldására ugyan több algoritmus is létezik, lényegében mindegyik a gradiens módszer egy változata.
...
Mi az Adaptív Momentum Becslés (\textit{Adaptive Moment Estimation, Adam~\cite{adam} }) módszert használtuk, mely a gradiens módszeren alapul.
...
Sűrű rétegről beszélünk, ha a bemenet és kimenet minden pontja között van kapcsolat. Általában egy aktivációs réteggel együtt használjuk. Ha $ x $ a réteg bemenete, $ \theta = \{W, \underline{b}\} $ a réteg súlyai, és $ f $ nemlinearitás, akkor a réteg modellje a következőképpen írható fel:
...
\begin{figure} [h!]
	...
	\begin{tikzpicture}[shorten >=1pt,->,draw=black!50, node distance=\layersep]
		...
		% Annotate the layers
		\node[annot,above of=H-1, node distance=1cm] (hl) {Sűrű réteg};
		\node[annot,left of=hl] {Bemenet};
		\node[annot,right of=hl] {Kimenet};
	\end{tikzpicture}
...
% TODO relu már nincs definiálva, majd máshol is csekkolni
...
Aktivációs réteg feladata, hogy valamilyen nem-linearitást vigyen a modellbe, ezáltal növelve a háló 
...
A $ softmax $ aktivációs függvényt osztályozásnál használjuk. A legnagyobb inputhoz 1 körüli outputot rendel, a többihez nullához közelit. Az eredmények összege pontosan 1.
 
\[  softmax(\underline{x})_j = \dfrac{e^{x_j}}{\sumn{i}e^{x_i}}  \]

% TODO softmax ábra kell? csak mert kicsit csúnyácska
...
% TODO konvolúció ábra kell? ha igen akkor sima, vagy neuronhálós?

Konvolúciós réteget általában kétdimenziós képekre használják, de létezik tetszőleges dimenziós változata is. Erre akkor is szükség van, ha a képünk nem szürkeárnyalatos, hanem több színcsatornával rendelkezik. Matematikailag két diszkrét jel konvolúciója egy- és kétdimenziós esetben:
...
\begin{itemize}
	...
	\item 
	\textbf{Full Padding}: Ha a két bemenetünk $ w_1 \times h_1 $ és $ w_2 \times h_2 $ méretű volt, akkor az eredmény ebben a módban $ (w_1 + w_2 - 1) \times (h_1 + h_2 - 1) $ méretű lesz. Ekkor minden ismeretlen adatot nullával helyettesítünk. Ebben a témakörben ezt jellemzően nem használjuk.
...
Mély hálóknál az a célunk, hogy a dimenzió folyamatosan csökkenjen. Erre jó módszer, hogy a képet feleakkorára\footnote{Gyakorlatban általában felezzük a képet, de lehet bármilyen más, egész számú hányados is.} csökkentjük, azaz négyesével összevonjuk a pixeleket, és valamilyen függvény segítségével kiszámoljuk belőlük az új értékeket.



Ha jellemzővektorokkal dolgozunk, akkor legtipikusabb a \textit{Max pooling} réteg, ami a négy pixel maximumát veszi, de használatos még az \textit{Average pooling} is, amely az értékeket átlagolja. Ennek akkor van jelentősége, ha színértékekből álló képet szeretnénk alul mintavételezni.
...
\paragraph{Felül-mintavételező réteg} (\textit{Upsampling})
...
Az autoencoder egy olyan speciális háló, amelynél a cél, hogy a bemenetet veszteségesen egy belső reprezentációra képezzük, majd ebből megpróbáljuk visszaállítani az eredeti adatot. Itt nincsenek címkéink, tehát ez egy nem felügyelt tanulás.
Tehát a \ref{sec:kanonikus.feladat}. pontban definiált általános tanulási feladat úgy módosul, hogy $ \underline{x} = \underline{y} $, azaz az elvárt kimenet megegyezik az elvárt bemenettel:
\[ 
...
% TODO általános alakra visszavezentni
%\noindent
...
Amikor a \ref{sec:kanonikus.feladat}. fejezetben definiáltuk a kanonikus tanulási feladatot, említettük, hogy az $ l(\underline{\hat{y}}, \underline{y}) $ függvényt költségfüggvénynek nevezzük. Nézzük most, hogy a dolgozatban miket használunk.
...
Bináris esetben $ \underline{y}=(0, 1) $ ha a mintha az egyik, és
$ \underline{y}=(1, 0) $ ha a másik csoportba tartozik.
...
Az előző két hiba arányát a Vevő működési karakterisztika (\textit{Receiver operating characteristic}) görbével szoktuk jellemezni. A két tengelyen az egyes hibák rátája található, és azt mutatja meg, hogy egy adott nagyságú elsőfajú hiba-rátához mekkora másodfajú tartozik. ROC-görbére a \ref{fig:roc.pelda}. ábrán láthatunk példát.
...
Ez a módszer arra való, hogy szabályozza a súlyok maximális méretét, és mérsékelje a túltanulást. A \ref{sec:kanonikus.feladat} fejezetbeli általános feladatban a $ R(\theta) $ függvény volt a regularizáció. Például az SVM feladatnál:

%Például, ahogy az SVM-nél láthattuk:


\[  J(\underline{w},b)  = \frac{1}{n} \sum\limits_{i=1}^{n} 
max(0, 1 - y_i(\underline{w} \cdot \underline{x}_i - b) + \lambda \norm{\underline{w}}  \]
...
A gépi tanuláshoz a \texttt{Keras} \footnote{\url{https://keras.io/}} és \texttt{Sklearn} \footnote{\url{http://scikit-learn.org/stable/modules/generated/sklearn.svm.SVC.html}} könyvtárakat 
használtuk. Ezek a \texttt{Tensorflow} \footnote{\url{https://www.tensorflow.org/}} csomag segítségével képesek a
...
% TODO kell ide a kód? vagy fölösleges
%
%\begin{lstlisting}  
%
%
...
Regularizációt\footnote{\url{http://scikit-learn.org/stable/auto_examples/svm/plot_svm_scale_c.html}} szabályozó paraméter. Ha visszatekintünk az SVM költségvényének alakjára a \ref{sec:svm}. fejezetben, akkor az \texttt{sklearn} $ C $ paramétere megegyeztethető $ \frac{1}{\lambda} $-val.
Ha $ C $ kicsi, akkor simább a döntési felület és egyszerűbb modell, ha nagyobb, akkor 
jobban próbál specializálni. Az első eset a túl nagy általánosság, a második a túltanulás
miatt lehet problémás.
...
% TODO konv lokális, dense nem
%TODO WTF az bal oldali szaggatott cucc?
% TODO convnet architektúra ábra a kód helyett:
% tikz, vagy rajz
...
Ahogy az korábban írtuk, a tanítás problémáját tulajdonképpen visszavezettük egy numerikus optimalizációs feladatra. Erre több módszer létezik, de általában a Gradiens Módszeren alapulnak. Mi az Adaptív Momentum Becslés (\textit{Adaptive Moment Estimation, Adam~\cite{adam} }) módszert használtuk. Erről bővebben a \ref{sec:adam}. fejezetben írtunk.
%
%\noindent
%Az algoritmus röviden:
%
...
%	\label{fig:roc.pelda}
%\end{figure}
% TODO innen ki let szedve egy ábra. Ellenőrizd hogy nem maradt e helyette hülyeség
...
Mi \texttt{WinPython 3.5.4}\footnote{\url{https://winpython.github.io/}} disztribúciót használtunk, de bármelyik másik 
...
Először is szükségünk van egy \texttt{CUDA}-t támogató videókártyára. Erre jelenleg csak
