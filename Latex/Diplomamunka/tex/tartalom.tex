...
A \textit{kanonikus gépi tanulási feladat} a következőképpen írható fel:
%\min\limits_{\theta} L(\theta, x_1, \dots, x_n, y_1, \dots, y_n) = \frac{1}{n}  \sumn{i} l(f(\theta, x_i), y_i) + R(\theta)

\[ 
\min\limits_{\underline{\theta}} L(\underline{\theta}, \underline{x}_1, \dots, \underline{x}_n, \underline{y}_1, \dots, \underline{y}_n) = \frac{1}{n}  \sumn i l(f(\underline{\theta}, \underline{x}_i), \underline{y}_i) + R(\underline{\theta})
 \]
,ahol $ \underline{x} $ a bemenetek, $ \underline{y} $ a hozzájuk tartozó elvárt kimenetek, $ f $ a modell, $ l $ a költségfüggvény, $ R $ a regularizáció, és $ \theta $ a tanulandó paraméterek. Ezekről a következő fejezetekben írunk részletesebben.
...
A \ref{fig:softmax}. ábrán szemléltetjük a Softmax működését. Két paramétert konstansnak választottunk: $ y=1 $ és $ z=2 $, és a harmadik érték függvényében mutatjuk a függvény eredményét.
